...
### `size_img.py`: A script that accesses the image files in the `img` folder, resizes them to be 128 x 128 pixels, and saves them in a folder called img_sized. If img_sized does not exist, it creates the folder first.

### `img`: Folder of 10 images downloaded from the image, unedited. 

### `img_resized`: The folder containing images accessed in iSpy.py to embed into the background image. Images are created by `size_img.py`
